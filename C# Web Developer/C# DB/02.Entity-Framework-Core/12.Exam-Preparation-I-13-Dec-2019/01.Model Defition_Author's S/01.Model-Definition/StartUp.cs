    ...
    public class StartUp
    {
        ...
        private static void ImportEntities(BookShopContext context, string baseDir, string exportDir)
        {
            ...
            var da = new
            {
                ...
                Author = context.Authors.ToList(),
                AuthorBook = context.AuthorsBooks.ToList()
            };

            var res = JsonConvert.SerializeObject(da);

            File.WriteAllText("../../../wow.txt", res);
        }
        ...
        private static void ExportEntities(BookShopContext context, string exportDir)
        {
            ...
            File.WriteAllText(exportDir + "Actual Result - ExportProjectWithTheirTasks.json", exportProcrastinatedProjects);
            ...
            File.WriteAllText(exportDir + "Actual Result - ExportMostBusiestEmployees.xml", exportTopMovies);
        }
