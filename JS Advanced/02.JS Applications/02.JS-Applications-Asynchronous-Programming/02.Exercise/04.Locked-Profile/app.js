async function lockedProfile() {
    ...
    Object.entries(data).forEach(p => {
        index++;
        const profileElement = createElement('div', '', ['class', 'profile']);

        const image = createElement('img', '', ['src', './iconProfile2.png', 'class', 'userIcon']);

        const lockedLabel = createElement('label', 'Lock');

        const lock = createElement('input', '', ['type', 'radio', 'name', `user${index}Locked`, 'value', 'lock', 'checked', '']);

        const unlockedLabel = createElement('label', 'Unlock');
        ...
        profileElement.appendChild(lockedLabel);
        profileElement.appendChild(lock);
        profileElement.appendChild(unlockedLabel);
        profileElement.appendChild(unlock);
        profileElement.appendChild(br);
        profileElement.appendChild(hr1);
        profileElement.appendChild(labelUsername);
        profileElement.appendChild(inputUsername);
        profileElement.appendChild(divUserHiddenFields);
