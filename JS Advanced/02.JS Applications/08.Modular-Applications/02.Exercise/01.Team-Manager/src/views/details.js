...
const memberTemplate = (request, isOwner) => html`
    <li>
        ...
        ${isOwner ? html`<a @click=${request.decline} href="javascript:void(0)" class="tm-control action">
            Remove from team
        </a>` : ''}
    </li>
`;
...
const pendingMemberTemplate = (request) => html`
    <li>
        ${request.user.username}
        <a @click=${request.approve} href="javascript:void(0)" class="tm-control action">
            Approve
        </a>
...
export async function detailsPage(ctx) {
    ...
    async function populateTemplate(teamId) {
        ...
        function createControls() {
            if (userId != null) {
                const request = requests.find(r => r._ownerId);
                if (isOwner) {
